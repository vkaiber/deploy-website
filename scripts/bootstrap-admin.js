...
rl.question("Admin email: ",email=>{
  ...
  rl.question("Admin password (min 12 chars): ",p=>{
    if(p.length<12){console.error("Password too short.");rl.close();process.exitCode=1;return;}
    const salt=crypto.randomBytes(16).toString("hex");
    const user={id:"usr_"+crypto.randomBytes(8).toString("hex"),email,hash:crypto.scryptSync(p,salt,64).toString("hex"),salt,role:"admin",badge:"admin.png",createdAt:new Date().toISOString(),banned:false,emailVerified:true};
    db.users=db.users.filter(u=>u.email!==email);db.users.push(user);
    fs.writeFileSync(file,JSON.stringify(db,null,2),"utf8");
    console.log("Admin account created:",email);rl.close();
  });
});
